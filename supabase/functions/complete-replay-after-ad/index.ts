...
serve(async (req) => {
  ...
  try {
    ...
    await supabase.from("credit_tx").insert({ idempotency_key, user_id: user.id, kind: "replay_story_ad", delta: 0, ref: { story_id } });
    await supabase.from("credits_ledger").insert({ user_id: user.id, delta: 0, reason: "replay_story_ad", meta: { story_id } });
    ...
    // Create session pointing to existing nodes (no node generation!)
