...
serve(async (req) => {
  ...
  try {
    ...
    const { data: activePass } = await supabase.from("access_passes").select("id")
      .eq("user_id", user.id).eq("target_type", "game").eq("target_id", story_id)
    ...
    const { data: game } = await supabase.from("public_games").select("*, story:stories(*)").eq("story_id", story_id).single();
    ...
    // Credit creator +1
    const { data: creatorProfile } = await supabase.from("profiles").select("credits, user_id").eq("user_id", game.creator_id).single();
    if (creatorProfile) {
      await supabase.from("profiles").update({ credits: creatorProfile.credits + 1 }).eq("user_id", game.creator_id);
      await supabase.from("credits_ledger").insert({ user_id: game.creator_id, delta: 1, reason: "play_public_game_royalty", meta: { story_id, player_id: user.id } });
    }

    await supabase.from("credit_tx").insert({ idempotency_key, user_id: user.id, kind: "play_public_game", delta: -10, ref: { story_id } });
    await supabase.from("credits_ledger").insert({ user_id: user.id, delta: -10, reason: "play_public_game", meta: { story_id } });
    ...
    await supabase.from("access_passes").insert({ user_id: user.id, target_type: "game", target_id: story_id, expires_at: expiresAt });

    // Create session pointing to existing story nodes (no node generation!)
    ...
    const { data: session, error: sessErr } = await supabase.from("story_sessions").insert({
      ...
      step: 0,
      current_node_id: "n0",
      state: { genre: storyData.genre, visited_nodes: [], chosen_choices: [] },
      ad_required: false,
    }).select().single();
