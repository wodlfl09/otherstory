...
serve(async (req) => {
  ...
  try {
    ...
    const { story_id, idempotency_key, ad_watched } = await req.json();
    ...
    if (existingTx) {
      // Already processed - find existing session
    ...
    if (plan === "free") {
      ...
      // Basic/Pro: use credits if >= 10, else ad
    ...
    if (useCredits) {
      ...
      // Record tx with 0 delta for ad-based replay
      await supabase.from("credit_tx").insert({
        idempotency_key,
        user_id: user.id,
    ...
    // Create new session (reuse original config)
    ...
    if (dailyLimit) {
      ...
      await supabase.from("replay_daily_limits").insert({
        user_id: user.id,
        day: today,
        count: 1,
      });
    ...
    // Generate first scene (invoke create-session logic would be complex, so create a minimal first scene)
    ...
    if (choicesList.length === 0) {
      for (let i = 0; i < choicesCount; i++) {
        choicesList.push({ id: `c${i}`, label: `선택지 ${i + 1}`, attitude: "neutral" });
      }
    }
